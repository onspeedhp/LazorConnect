...
class BackpackWallet {
  ...
  public connect(): string {
    ...
    // We'll use our web URL and add the backpack parameter to identify the response type
  ...
  public handleConnectResponse(url: URL): string | null {
    try {
      console.log(
        "Handling Backpack connect response, URL params:",
        Object.fromEntries(url.searchParams.entries()),
      );

      // Extract response parameters
      const data = url.searchParams.get("data");
      const wallet_encryption_public_key = url.searchParams.get(
      ...
      const direct_wallet_public_key =
        url.searchParams.get("wallet_public_key");
      ...
      if (data && wallet_encryption_public_key && nonce && this.session) {
        ...
        try {
          // Decode parameters
          const walletEncPubKeyBytes = base58ToArray(
            wallet_encryption_public_key,
          );
          ...
          console.error(
            "Error decrypting Backpack connect response:",
            decryptError,
          );
        }
      }
      ...
      console.error(
        "Missing or invalid parameters in Backpack connect response",
      );
      return null;
    } catch (error) {
  ...
  public signAndSendTransaction(transaction: Transaction): string {
    try {
      ...
      // Backpack will append its response parameters to this URL
  ...
  public handleTransactionResponse(url: URL): string | null {
    try {
      ...
      if (data && nonce && wallet_encryption_public_key) {
        try {
          // Decode parameters
          const walletPublicKeyBytes = base58ToArray(
            wallet_encryption_public_key,
          );
          const nonceBytes = base58ToArray(nonce);
          const encryptedDataBytes = base58ToArray(data);
      ...
      console.error(
        "Missing or invalid signature in Backpack transaction response",
      );
      return null;
    } catch (error) {
